...
class TestTaxCalculations:
    ...
    def test_simple_tax_calculation(self, test_db, test_user, complete_tax_data):
        ...
        # Add income
        income = IncomeSource(user_id=test_user.id, source_type="Salary", annual_amount=300000, tax_year=tax_year)
        test_db.add(income)
        test_db.commit()
    ...
    def test_tax_calculation_with_deductions(self, test_db, test_user, complete_tax_data):
        ...
        # Add income
        income = IncomeSource(user_id=test_user.id, source_type="Salary", annual_amount=400000, tax_year=tax_year)
        test_db.add(income)
    ...
    def test_provisional_tax_calculation(self, test_db, test_user, complete_tax_data):
        ...
        income = IncomeSource(
            user_id=test_user.id, source_type="Consulting", annual_amount=500000, is_paye=False, tax_year=tax_year
        )
        test_db.add(income)
        test_db.commit()
    ...
    def test_provisional_tax_due_dates(self, test_db, test_user, complete_tax_data):
        ...
        # Add income
        income = IncomeSource(user_id=test_user.id, source_type="Freelance", annual_amount=300000, tax_year=tax_year)
        test_db.add(income)
        test_db.commit()
