...
class TestPerformance:
    ...
    def test_provisional_tax_calculation_performance(self, test_db, test_user, complete_tax_data):
        ...
        assert calculation_time < 0.5, f"Provisional tax calculation took {calculation_time:.3f} seconds, expected < 0.5"
    ...
    def test_large_dataset_performance(self, test_db, complete_tax_data):
        ...
        for i in range(50):
            income = IncomeSource(
                user_id=user.id,
                source_type=f"Income_{i}",
                annual_amount=10000 + (i * 1000),
                tax_year=tax_year
            )
            test_db.add(income)
