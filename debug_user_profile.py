    ...
    try:
        ...
        if user.is_provisional_taxpayer and income_sources:
            ...
            try:
                ...
                print(f"   First Payment (Due: {prov_tax_result['first_payment']['due_date']}): R{prov_tax_result['first_payment']['amount']:,.2f}")
                print(f"   Second Payment (Due: {prov_tax_result['second_payment']['due_date']}): R{prov_tax_result['second_payment']['amount']:,.2f}")
...
def list_all_users():
    ...
    try:
        ...
        for user in users:
            ...
            print(f"   ID: {user.id}, Email: {user.email}, Name: {user.name} {user.surname}, {prov_status} {admin_status}")
