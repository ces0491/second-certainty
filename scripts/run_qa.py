...
class TestResult:
    ...
    def add_result(self, passed: bool, test_name: str, duration: float = 0, error: str | None = None):
        if passed:
            self.passed += 1
        else:
            self.failed += 1
...
def run_command(command: str, description: str, timeout: int = 60, capture_output: bool = True) -> tuple[bool, str, float]:
    ...
    try:
        ...
        if result.stdout and result.stdout.strip() and len(result.stdout) < 500:
            print(f"   Output: {result.stdout.strip()}")
        
        return True, result.stdout or "", duration
...
def generate_report(result: TestResult, quality_results: dict[str, bool], test_results: dict[str, bool], 
                   app_results: dict[str, bool], coverage_percentage: float) -> None:
